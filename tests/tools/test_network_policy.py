...
def test_load_network_allowlist_missing_section_returns_defaults(tmp_path):
    ...
    # auto_allow_providers defaults to True so rules will be populated from
    # PROVIDER_REGISTRY; only user-configured rules should be absent.
    user_rules = [r for r in policy["rules"] if not r.get("source", "").startswith("auto:")]
    assert user_rules == []
